...
namespace ParkitectAssetEditor.UI
{
	...
	public class AssetEditorWindow : EditorWindow
	{
		...
		private void DrawAssetDecoDetailSection()
		{
			...
			{
				_selectedAsset.MinSize = Mathf.RoundToInt(EditorGUILayout.Slider("Min size: ", _selectedAsset.MinSize, 0.1f, _selectedAsset.MaxSize) * 20f) / 20f;
				_selectedAsset.MaxSize = Mathf.RoundToInt(EditorGUILayout.Slider("Max size: ", _selectedAsset.MaxSize, _selectedAsset.MinSize, 10) * 20f) / 20f;
			}
